...
void ConvertToGrayScale( BYTE* pBitmapBits,  
                       LONG lWidth,  
                       LONG lHeight
					                           )  
{  
    ...
    {
        for ( int x = 0; x < 2 * lWidth; x+=2 )
        {
            // Determine positions in original and padded buffers
            long bufpos = y * 2 * lWidth + ( 2 * lWidth - x );     
             
             
            // Swap R&B, G remains, swap B&R
            pBitmapBits[ bufpos ] = pBitmapBits[ bufpos]*0.50+ pBitmapBits[ bufpos + 1 ]*0.50 ; 
            pBitmapBits[ bufpos + 1 ] = 0;
